...
• /start {json} o solo /start - Iniciar servicio (auto usa .env/DB)
...
🔌 WebSocket: {'✅ Conectado' if ws_connected else '❌ Desconectado'}"""
    ...
    def _start(m):
        try:
            ...
            # Si no hay JSON, usar iniciar_auto
            if not payload:
                bot.send_message(m.chat.id, "🚀 Iniciando automáticamente con parámetros de .env/DB...", reply_markup=menu)
                res = _post("/cotizaciones/iniciar_auto")
            else:
            ...
            if ok:
                ...
                error_msg = res.get("message", "Error desconocido") if isinstance(res, dict) else str(res)
    ...
    def _stop(m):
        try:
            bot.send_message(m.chat.id, "🛑 Deteniendo servicio...", reply_markup=menu)
            res = _post("/cotizaciones/detener")
            ok = (isinstance(res, dict) and res.get("status") == "success")
            bot.send_message(m.chat.id, ("✅ Servicio detenido" if ok else f"❌ Error: {res}"), reply_markup=menu)
        except Exception as e:
            bot.send_message(m.chat.id, f"❌ Error: {str(e)}", reply_markup=menu)
    ...
    def _restart(m):
        try:
            bot.send_message(m.chat.id, "🔄 Reiniciando servicio...", reply_markup=menu)
            res = _post("/cotizaciones/reiniciar")
            ok = (isinstance(res, dict) and res.get("status") == "success")
            bot.send_message(m.chat.id, ("✅ Servicio reiniciado" if ok else f"❌ Error: {res}"), reply_markup=menu)
        except Exception as e:
            bot.send_message(m.chat.id, f"❌ Error: {str(e)}", reply_markup=menu)
